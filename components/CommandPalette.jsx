...
const commands = [
  { label: "Who is Bruno?", href: "#about" },
  { label: "Show flagship project", href: "#work" },
  { label: "Show AI projects", href: "#work" },
  { label: "Show technologies", href: "#technologies" },
  { label: "Why Québec?", href: "#about" },
  { label: "Contact Bruno", href: "#contact" }
];
        ...
        {open ? (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-start justify-center bg-black/75 px-4 pt-[14vh] backdrop-blur-md" onMouseDown={() => setOpen(false)}>
            <motion.div initial={{ opacity: 0, y: -20, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -12, scale: 0.98 }} onMouseDown={(e) => e.stopPropagation()} className="w-full max-w-xl overflow-hidden rounded-2xl border border-white/15 bg-[#0a0a0b] shadow-[0_30px_120px_rgba(0,0,0,0.8)]">
              <div className="flex items-center gap-3 border-b border-white/10 p-4"><Command className="h-4 w-4 text-champagne" /><input autoFocus value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Type a command…" className="min-w-0 flex-1 bg-transparent text-base text-white outline-none placeholder:text-inkMute" /><button onClick={() => setOpen(false)} aria-label="Close"><X className="h-4 w-4 text-inkMute" /></button></div>
              <div className="p-2">
                {hire ? <div className="rounded-xl border border-champagne/25 bg-champagne/10 p-5"><p className="font-display text-xl font-bold text-white">Excellent command. 😄</p><p className="mt-2 text-sm text-inkSoft">Let’s talk.</p><a href="mailto:brunoeliassantos097@gmail.com" className="mt-4 inline-flex items-center gap-2 text-sm font-bold text-champagne"><Mail className="h-4 w-4" /> Email Bruno</a></div> : filtered.map((command) => <button key={command.label} onClick={() => go(command.href)} className="flex w-full items-center justify-between rounded-xl px-4 py-3 text-left text-sm font-semibold text-inkSoft transition hover:bg-white/[0.06] hover:text-white"><span>&gt; {command.label}</span><ArrowRight className="h-4 w-4 text-champagne" /></button>)}
                {!hire && !filtered.length ? <p className="p-5 text-sm text-inkMute">No command found. Try “hire bruno”.</p> : null}
              </div>
              <div className="border-t border-white/10 px-4 py-3 font-mono text-[9px] uppercase tracking-[0.2em] text-inkMute">Tip: there may be a hidden command.</div>
            </motion.div>
          </motion.div>
        ) : null}
      </AnimatePresence>
